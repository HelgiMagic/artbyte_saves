								...
								foreach($arResult["ITEMS"] as $key=>$arItem)
								{
									...
									<div class="products-sidebar-item">
										...
										<div class="products-sidebar-item-list-wrapp" <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>style="display:block;"<?endif;?>>
											...
											<?
														?>
                                        <ul class="products-sidebar-item-list ul-filter-listnew">
                                            <?php $i = 0; ?>
                                            <?php foreach($arItem["VALUES"] as $val => $ar): ?>
                                                <li <?php echo ($i >= 6) ? 'style="display:none;"' : ''; ?>>
                                                    <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="checkbox-label products-filter-checkbox-label <?// echo $ar["DISABLED"] ? 'disabled': '' ?>">
                                                        <input type="checkbox" class="checkbox startfiltersearch" value="<?php echo $ar["HTML_VALUE"] ?>" name="<?php echo $ar["CONTROL_NAME"] ?>" id="<?=$ar["CONTROL_ID"];?>" <?php echo $ar["CHECKED"] ? 'checked="checked"' : ''; ?>/>
                                                        <span class="products-filter-checkbox-title"><?=$ar["VALUE"];?> <?=$arItem["HINT"]?></span>
                                                        <?php if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])): ?>
                                                            <span data-role="count_<?=$ar["CONTROL_ID"]?>" class="products-filter-checkbox-count"><?php echo $ar["ELEMENT_COUNT"]; ?></span>
                                                        <?php endif; ?>
                                                    </label>
                                                </li>
                                                <?php $i++; ?>
                                        ...
                                        <?php
                                        if (isset($_COOKIE['testimTemku'])) {
                                            $value = $_COOKIE['testimTemku'];
                                            // Обязательно декодируем, если кодировали в JS
                                            $value = urldecode($value);
                                            echo "Значение сессионной куки: " . htmlspecialchars($value);
                                        } else {
                                            echo "Сессионная кука testimTemku не найдена";
                                        }
                                        ?>
