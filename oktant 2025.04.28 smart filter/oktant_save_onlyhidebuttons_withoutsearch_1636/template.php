...
<script>
window.toggleStates = <?= json_encode($toggleStates, JSON_UNESCAPED_SLASHES) ?>;
var maxVisible  = 6;
</script>
...
//выпиливаем ползунок с ценами конец!!!
								...
								foreach($arResult["ITEMS"] as $key=>$arItem)
								{
									...
									<div class="products-sidebar-item">
                                        ...
                                        <div class="product-sidebar-search-wrapper">
                                            <input type="text" class="product-sidebar-search" placeholder="Поиск" />
										...
										<div class="products-sidebar-item-list-wrapp" <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>style="display:block;"<?endif;?>>
                                            ...
                                            <ul
                                                ...
                                                <?php foreach ($arItem["VALUES"] as $val => $ar): ?>
                                                    <?php
                                                    // Решаем, показывать ли элемент
                                                    $isExpanded = !empty($toggleStates[$key]);
                                                    $shouldHide = ($i >= $maxVisible && !$isExpanded);
                                                    ?>
      ...
      <?php if (count($arItem["VALUES"]) > $maxVisible): ?>
        <?php
          ...
          $expanded = !empty($toggleStates[$key]);
        ?>
        <button
          type="button"
