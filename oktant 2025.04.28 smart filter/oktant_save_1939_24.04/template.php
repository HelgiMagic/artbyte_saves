...
$maxVisible  = 6;
$hideULStates  = [];
if (isset($_COOKIE['hideULStates'])) {
    $decoded = json_decode($_COOKIE['hideULStates'], true);
    if (is_array($decoded)) {
        $hideULStates = $decoded;
    }
}

// ++ НАЧАЛО: Добавленный код для чтения состояния поиска ++
$searchFilterStates = [];
if (isset($_COOKIE['searchFilterStates'])) { // Имя cookie для состояния поиска
    $decoded = json_decode($_COOKIE['searchFilterStates'], true);
    if (is_array($decoded)) {
        $searchFilterStates = $decoded;
    }
}
// ++ КОНЕЦ: Добавленный код для чтения состояния поиска ++

$hideWrapperStates  = [];
if (isset($_COOKIE['hideWrapperStates'])) {
    $decoded = json_decode($_COOKIE['hideWrapperStates'], true);
    if (is_array($decoded)) {
        $hideWrapperStates = $decoded;
    }
}
...
<script>
// Передаем состояния из PHP в JS
window.hideULStates = <?= json_encode($hideULStates, JSON_UNESCAPED_SLASHES | JSON_FORCE_OBJECT) ?>; // Используем FORCE_OBJECT для надежности
// ++ НАЧАЛО: Добавлена передача состояния поиска ++
window.searchFilterStates = <?= json_encode($searchFilterStates, JSON_UNESCAPED_SLASHES | JSON_FORCE_OBJECT) ?>;
// ++ КОНЕЦ: Добавлена передача состояния поиска ++
var maxVisible  = 6; // Максимальное видимое количество элементов по умолчанию
</script>
...
//выпиливаем ползунок с ценами конец!!!
								...
								<?php
									// --- НАЧАЛО ИЗМЕНЕНИЙ ---
									// Получаем ключ текущего элемента фильтра
									$itemKey = $key; // Используем существующий $key ($arItem["ENCODED_ID"])

									// Проверяем состояние в куках hideWrapperStates
									// true в куке означает, что элемент должен быть СКРЫТ
									$isHidden = isset($hideWrapperStates[$itemKey]) && $hideWrapperStates[$itemKey] === true;

									// Определяем классы и стили на основе состояния из куки
									$titleClass = "products-sidebar-item-title";
									$listWrapperStyle = "";
									$searchWrapperStyle = "";
									$buttonWrapperStyle = ""; // Добавлено для управления видимостью кнопок

									if (!$isHidden) {
										// Если НЕ скрыто (т.е. должно быть показано)
										$titleClass .= " active";
										$listWrapperStyle = "display:block;";
										// Поиску и кнопкам не нужен display:block, если они видимы по умолчанию
									} else {
										// Если скрыто
										// Класс active не добавляем
										$listWrapperStyle = "display:none;";
										$searchWrapperStyle = "display:none;";
										$buttonWrapperStyle = "display:none;"; // Скрываем и кнопки
									}
									// --- КОНЕЦ ИЗМЕНЕНИЙ ---
									?>
									<div class="products-sidebar-item" data-item-key="<?= htmlspecialchars($itemKey) ?>"> <?/* Добавляем data-атрибут с ключом */?>
										<div class="<?= $titleClass ?>"> <?/* Применяем класс */?>
                                        ...
                                        <? // ++ НАЧАЛО: Добавлен блок поиска с учетом состояния из cookie ++ ?>
                                        <?php
                                        // Получаем сохраненное значение поиска для текущего свойства ($key)
                                        $currentSearchValue = isset($searchFilterStates[$key]) ? htmlspecialchars($searchFilterStates[$key], ENT_QUOTES) : '';
                                        // Определяем, показывать ли иконку очистки
                                        $showClearIcon = !empty($currentSearchValue) ? '' : 'style="display: none;"';
                                        ?>
                                        <div class="product-sidebar-search-wrapper" style="<?= $searchWrapperStyle ?>"> <?/* Применяем стиль */?>
                                            <input
                                                type="text"
                                                class="product-sidebar-search"
                                                placeholder="Поиск"
                                                data-list-index="<?= $key ?>" <?/* Оставляем для логики поиска */?>
                                                value="<?= $currentSearchValue ?>" <?/* Устанавливаем значение из cookie */?>
                                            />
                                            <span class="clear-icon" <?= $showClearIcon ?> <?/* Показываем/скрываем иконку */?>>
                                        ...
                                        <? // ++ КОНЕЦ: Добавлен блок поиска ++ ?>
										
										<div class="products-sidebar-item-list-wrapp" <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?> style="<?= $listWrapperStyle ?>"<?endif;?>>
											...
											<?
														?>
                                           <?php
                                            // Получаем значение поиска для текущего свойства ($key), если оно есть
                                            $currentSearchValue = isset($searchFilterStates[$key]) ? trim($searchFilterStates[$key]) : '';
                                            $currentSearchValueLower = !empty($currentSearchValue) ? mb_strtolower($currentSearchValue, 'UTF-8') : ''; // В нижний регистр для поиска без учета регистра
                                            ?>
                                            <ul
                                            ...
                                            >
                                                ...
                                                <?php foreach ($arItem["VALUES"] as $val => $ar): ?>
                                                    <?php
                                                    // 1. Проверяем соответствие поиску
                                                    $matchesSearch = true; // По умолчанию считаем, что соответствует
                                                    if (!empty($currentSearchValueLower)) {
                                                        $itemValueLower = mb_strtolower($ar["VALUE"], 'UTF-8');
                                                        if (mb_strpos($itemValueLower, $currentSearchValueLower) === false) {
                                                            $matchesSearch = false; // Не соответствует
                                                        }
                                                    }

                                                    // 2. Проверяем, нужно ли скрывать по логике "Показать все/Скрыть"
                                                    $isExpanded = !empty($hideULStates[$key]);
                                                    $shouldHideByLimit = ($i >= $maxVisible && !$isExpanded);

                                                    // 3. Определяем финальный стиль display
                                                    $liStyle = '';
                                                    if (!$matchesSearch || $shouldHideByLimit) {
                                                        // Скрываем, если НЕ соответствует поиску ИЛИ если превышен лимит и список не развернут
                                                    ...
                                                    <li style="<?= $liStyle ?>">
                                                        <label
                                                            ...
                                                            class="checkbox-label products-filter-checkbox-label"
                                                            <?/* Добавляем data-text для JS фильтрации, чтобы точно знать текст */?>
                                                            data-text="<?= htmlspecialchars(mb_strtolower($ar["VALUE"], 'UTF-8'), ENT_QUOTES) ?>"
                                                        >
                                                            <input
                                                                type="checkbox"
                                                                ...
                                                                <?= $ar["CHECKED"] ? 'checked' : '' ?>
                                                            />
                                                            <span class="products-filter-checkbox-title">
                                                                <?= htmlspecialchars($ar["VALUE"]) ?>
                                                            </span>
                                                            <?php if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])): ?>
                                                            <span
                                                                data-role="count_<?= $ar["CONTROL_ID"] ?>"
                                                                class="products-filter-checkbox-count"
                                                            >
                                                                (<?= $ar["ELEMENT_COUNT"] ?>) <?/* Добавил скобки для ясности */?>
                                                            </span>
                                                            <?php endif; ?>
                                                        </label>
                                                    </li>
      ...
      <!-- Кнопка "Сбросить" оставляем без изменений -->
      <button
        type="button"
        class="reset-filter-button"
        style="color: <?= $containerChecked ? '#0055C7' : '#888D96' ?>"
      >
        <svg width="9" height="10" viewBox="0 0 9 10" fill="none" xmlns="http://www.w3.org/2000/svg">
          <line x1="0.353553" y1="0.646569" x2="8.57794" y2="8.87095" stroke="<?= $containerChecked ? '#0055C7' : '#888D96' ?>"/>
          <line x1="0.421837" y1="8.87081" x2="8.64622" y2="0.646428" stroke="<?= $containerChecked ? '#0055C7' : '#888D96' ?>"/>
        </svg>
        Сбросить выбор<?= $containerChecked ? " ({$arItem['CHECKED_COUNT']})" : '' ?>
      </button>

      <?php if (count($arItem["VALUES"]) > $maxVisible): ?>
        <?php
          // Определяем, нужно ли изначально показывать все
          $expanded = !empty($hideULStates[$key]);
        ?>
        <button
          type="button"
