...
<?if($arResult["ID"]):?>
	...
	<?if($templateData['LINK_SERVICES_NEW'] && !$templateData['OUT_OF_PRODUCTION']):?>
	
		<?//buy_services in anounce	
		ob_start();		
			$GLOBALS['arBuyServicesFilter']['ID'] = $templateData['LINK_SERVICES_NEW'];
			...
			<?$APPLICATION->IncludeComponent(
				...
				"services_list_NEW",
				[
					'BASE_PRODUCT_PRICE' => $templateData['MIN_PRICE'],
					'BASE_PRODUCT_ID' => $arResult['ID'],
					'IBLOCK_ID' => $arParams['IBLOCK_SERVICES_NEW_ID'],
					'PRICE_CODE' => $arParams['PRICE_CODE'],
	...
	<?$i = 0;
	//echo "<pre>"; print_r($templateData['VIDEO']); echo "</pre>";
	//echo "<pre>"; print_r($templateData['VIDEO_IFRAME']); echo "</pre>";
	...
	$bShowDeliveryTab = (($arParams["SHOW_DELIVERY"] == "Y" && $arResult['HIDE_PAINT']== false) ? ++$i : '');
...
<?//complects?>
<h3 class="font_lg">Добавить сопутствующие товары со скидкой</h3>

<? if(\Bitrix\Main\Loader::includeModule('clickon.set')){ ?>
   <? $APPLICATION->IncludeComponent('clickon.set:set.element', 'custom_artbyte_new', [
      'ELEMENT_ID' => $arResult["ID"],
      'BASKET_URL' => $arParams['BASKET_URL'],
			...
			<?
				...
				<div class="ordered-block <?=$code?>">
							...
							<div class="tabs arrow_scroll bottom-line" data-plugin-options='{"axis": "x", "scrollInertia": 200, "snapAmount": 70, "scrollButtons": {"enable": true}}'>
								<ul class="nav nav-tabs">
									<?if($bShowExpandablesTab):?>
										<li class="active"><a href="#expandables" data-toggle="tab" class="linked"><?=$arParams["DETAIL_EXPANDABLES_TITLE"];?></a></li>
									<?endif;?>
									<?if($bShowAssociatedTab):?>
										<li class="<?=$bShowExpandablesTab ? '' : 'active'?>"><a href="#assoc" data-toggle="tab" class="linked"><?=$arParams["DETAIL_ASSOCIATED_TITLE"];?></a></li>
									<?endif;?>
								</ul>
							</div>
					...
					<?if($templateData['ASSOCIATED']):?>
						...
						<?if($bShowAssociatedTab):?>
							<?if($bNavTabs):?>
								<div class="tab-pane <?=(!$templateData['EXPANDABLES'] ? "active cur" : "");?>" id="assoc">
							<?else:?>
								<div class="ordered-block__title option-font-bold font_lg">
