...
$arResult['NEW_SERVICES'] = $arResult['PROPERTIES']['NEW_SERVICES']['VALUE'];
...
if(!empty($arResult['DISPLAY_PROPERTIES']))
{
	...
	if(isset($arResult["PROPERTIES"]["VIDEO_YOUTUBE"]["VALUE"]))
	{
		if(is_array($arResult["PROPERTIES"]["VIDEO_YOUTUBE"]["VALUE"]))
			$arVideo = $arVideo + $arResult["PROPERTIES"]["VIDEO_YOUTUBE"]["~VALUE"];
		elseif(strlen($arResult["PROPERTIES"]["VIDEO_YOUTUBE"]["VALUE"]))
			$arVideo[] = $arResult["PROPERTIES"]["VIDEO_YOUTUBE"]["~VALUE"];
	}

	if(isset($arResult["PROPERTIES"]["VIDEO_IFRAME"]["VALUE"]))
	{
		if(is_array($arResult["PROPERTIES"]["VIDEO_IFRAME"]["VALUE"]))
			$arVideo = array_merge($arVideo, $arResult["PROPERTIES"]["VIDEO_IFRAME"]["~VALUE"]);
		elseif(strlen($arResult["PROPERTIES"]["VIDEO_IFRAME"]["VALUE"]))
			$arVideo[] = $arResult["PROPERTIES"]["VIDEO_IFRAME"]["~VALUE"];
	}
...
/*Скрываем таб покарски в завиисмости от свойства категории товаров*/
$hide_paint = false;
...
$rsSection = $entity::getList(array(
    ...
    "select" => array("ID","UF_HIDEPAINT"),

	))->fetchAll();

if (isset($rsSection[0]["UF_HIDEPAINT"]) && $rsSection[0]["UF_HIDEPAINT"]=="1" ){
$hide_paint = true;
}

if (!$hide_paint && $arResult['PROPERTIES']['TAGS_PHOTO']['VALUE']) {
	$photosTags = \Bitrix\Iblock\Elements\ElementPhotosGalleryTable::getList([
		'select' => ['ID', 'NAME', 'PREVIEW_PICTURE', 'TAGS_PHOTO'],
		'filter' => [
			'TAGS_PHOTO.VALUE' => $arResult['PROPERTIES']['TAGS_PHOTO']['VALUE']
		],
	])->fetchCollection();
	$arPhotos = [];
	foreach ($photosTags as $photos) {
		$arPhotos[$photos->getId()] = CFile::GetPath($photos->getPreviewPicture());
	}
	if (!empty($arPhotos)) {
		$arResult['PHOTOS_TAGS'] = $arPhotos;
	}
}
...
if (is_object($cp))
{
    $cp->arResult['HIDE_PAINT'] = $hide_paint;
    $cp->SetResultCacheKeys(array('HIDE_PAINT'));
}
$arResult['linkedProducts'] = WoodenLinkProducts::getLinkedProductsByIds([$arResult['ID']]);

if ($arResult['linkedProducts']['PRODUCTS_IDS']) {
	$arSelect = Array("ID", "NAME","DETAIL_PAGE_URL");
	$arFilter = Array("IBLOCK_ID"=>$arParams['IBLOCK_ID'], "ACTIVE"=>"Y", 'ID' =>$arResult['linkedProducts']['PRODUCTS_IDS']);       
	$res = CIBlockElement::GetList(Array('SORT' => 'ASC'), $arFilter, false, false, $arSelect);
	while($ob = $res->GetNext()){ 
		$arResult['CUSTOM_LINKED_PRODUCTS'][$ob['ID']] = $ob;
	}
}
