(function (window) {
  ...
  window.JCCatalogItem.prototype = {
    ...
    openBuyPopup: function (id) {
      ...
      console.log('open boy popup', id);

      const buyPopup = document.querySelector('.buy-item-popup');
      let popupWrapper = buyPopup;
      ...
      //Создаём контент и добавляем
      const closeButton = document.createElement('div');
      closeButton.classList.add('close');
      closeButton.textContent = 'X'; // Крестик для закрытия
      ...
      //Цвета
      console.log(this.treeProps, this.offers);
      if (this.treeProps && this.treeProps.length > 0) {
        console.log('treeprops variant');
        const colorId = 254;
        const sizeId = 264;
        //Проверка и получение свойств из компонента
        this.treeProps.forEach((prop) => {
          if (prop.ID == colorId) {
            //Находим свойство "Цвет", проверка на CODE
            const colorTitle = document.createElement('div');
            colorTitle.textContent = 'Выберите цвет';
            popupWrapper.appendChild(colorTitle);

            const colorsContainer = document.createElement('div');
            colorsContainer.classList.add('colors');
            popupWrapper.appendChild(colorsContainer);

            const valuesArray = Object.values(prop.VALUES);

            valuesArray.forEach((color) => {
              //Перебираем значения свойства "цвет"
              const colorOption = document.createElement('div');
              colorOption.classList.add('color');
              colorOption.style.backgroundColor = color.PICT ? `url(${color.PICT.SRC})` : color.NAME; //Цвет в виде картинки или текстового названия.
              colorOption.dataset.propId = prop.ID;
              colorOption.dataset.valueId = color.ID;
              colorOption.dataset.treevalue = `${prop.ID}_${size.ID}`;
              colorOption.title = color.NAME; //Тултип

              colorOption.addEventListener('click', (event) => {
                //Снимаем выдление с других
                colorsContainer.querySelectorAll('.color').forEach((c) => c.classList.remove('selected'));
                colorOption.classList.add('selected');
                this.selectOfferPropFromPopup(prop.ID, color.ID);
              });
          ...
          // Размеры.
          if ((prop.ID = sizeId)) {
            // Находим свойство "Размер"
            const sizeTitle = document.createElement('div');
            sizeTitle.textContent = 'Выберите размер';
            popupWrapper.appendChild(sizeTitle);

            const sizesContainer = document.createElement('div');
            sizesContainer.classList.add('sizes');
            popupWrapper.appendChild(sizesContainer);

            const valuesArray = Object.values(prop.VALUES);

            valuesArray.forEach((size) => {
              // Перебираем варианты размеров
              const sizeOption = document.createElement('div');
              sizeOption.classList.add('size');
              sizeOption.textContent = size.NAME;
              sizeOption.dataset.propId = prop.ID;
              sizeOption.dataset.valueId = size.ID;
              sizeOption.dataset.treevalue = `${prop.ID}_${size.ID}`;
              sizeOption.title = size.NAME;
              sizeOption.addEventListener('click', (event) => {
                //Снимаем выдление с других
                sizesContainer.querySelectorAll('.size').forEach((s) => s.classList.remove('selected'));
                sizeOption.classList.add('selected');
                this.selectOfferPropFromPopup(prop.ID, size.ID);
              });
      ...
      } else if (this.offers && this.offers.length > 0) {
        //Если нет treeProps берем данные из offers

        let colors = new Set(); //Используем Set чтобы цвета не повторялись
        let sizes = new Set();
        //Собираем цвета
        this.offers.forEach((offer) => {
          if (offer.PROPERTIES && offer.PROPERTIES.COLOR_REF && offer.PROPERTIES.COLOR_REF.VALUE) {
            colors.add(offer.PROPERTIES.COLOR_REF.VALUE);
          }
          if (offer.PROPERTIES && offer.PROPERTIES.SIZE_REF && offer.PROPERTIES.SIZE_REF.VALUE) {
            sizes.add(offer.PROPERTIES.SIZE_REF.VALUE);
          }
        });
        ...
        if (colors.size > 0) {
          ...
          colors.forEach((color) => {
            const colorOption = document.createElement('div');
            colorOption.classList.add('color');
            colorOption.style.backgroundColor = color; //Предполагаем, что color - это строковое значение цвета, типа "red", "#ff0000"
            colorOption.dataset.value = color; //Используем для определения offer-a
            colorsContainer.appendChild(colorOption);
          });
        }
      ...
      addToCartButton.addEventListener('click', () => {
        // this.add2Basket();  //Добавляем в корзину

        //После выбора добавляем и сразу закрываем попап.
        if (this.productType === 3 && (!this.selectedValues || Object.keys(this.selectedValues).length === 0)) {
          //Проверка, выбран ли SKU
          alert('Пожалуйста, выберите опции товара.'); //Если опции не выбраны (для SKU)
          return;
        }
        this.add2Basket();
        popupWrapper.remove(); //Закрываем после добавления
      });
      popupWrapper.appendChild(addToCartButton);
    },
    //Вспомогательная функция, эмулирующая выбор
    selectOfferPropFromPopup: function (propId, valueId) {
      const strTreeValue = `PROP_${propId}_${valueId}`;
      //   const target = {
      //     hasAttribute: (attr) => attr === 'data-treevalue',
      //     getAttribute: (attr) => (attr === 'data-treevalue' ? strTreeValue : null), //Создаем "целевой" обьект для вызова "родной функции"
      //     parentNode: this.obTree.querySelector(`[data-entity="sku-line-block"][data-prop-id="${propId}"] ul`), // !!! ВАЖНО: Эмулируем parentNode
      //   };
