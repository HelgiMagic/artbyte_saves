...
if (isset($arResult['ITEM']))
{
    ...
    <?
    /**
     * Get current catalog view
     * COOKIE
     */
    $currentView = $_COOKIE['BX_VIEW'];
    $shopCatClass = '';
    if ($currentView == 'TILE') {
        $shopCatClass = '-tile';
    }
    ?>
<div class="shop-cat<?/*=$shopCatClass; */?>__item" id="<?=$areaId?>" data-entity="item"  data-ELEMENT_ID=<?=$arResult['ITEM']["ID"]?>>
	<!--<div class="product-item-container<?/*=(isset($arResult['SCALABLE']) && $arResult['SCALABLE'] === 'Y' ? ' product-item-scalable-card' : '')*/?>"
		>-->
		<?

		$documentRoot = Main\Application::getDocumentRoot();
#		$templatePath = strtolower($arResult['TYPE']).'/template.php'; // CARD
		$templatePath = 'card/template.php'; // CARD
...
<script>
	$(document).on('click', '.product-item-scu-item-text-container', function () {
		const priceOld = $(this).data('meterprice');
		const priceNew = $(this).data('meterprice_new');

		$(this).closest('.shop-cat__content').find('.price2 .old').html(priceOld);
		$(this).closest('.shop-cat__content').find('.price2 .new').html(priceNew);
	});
</script>