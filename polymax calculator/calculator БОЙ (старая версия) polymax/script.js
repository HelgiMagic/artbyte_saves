...
function findClosestDivisor(target, start, precision = 4) {
  for (let i = start; ; i++) {
    ...
    const test = Number(result.toFixed(precision));
...
function renderResults() {
  ...
  const remains = detailsSquares.map((listCutWidth) => {
    ...
    if (sizes[1]) {
      ...
      console.log('listSquare:', listSquare);
      console.log('listCutWidth:', listCutWidth);
...
function renderActualListDetails(fatherHTML, details, listI, isMiniature) {
  ...
  if (!methodValue.startsWith('Разрезать')) {
    detailsToRender = sortDetails(detailsWithQuantityFill, remainingOrientation);
  }
  ...
  detailsToRender.forEach((row, rowI) => {
    ...
    row.forEach((detail, i) => {
      ...
      // topValue и leftValue для метода Оптимально
      const topValue = removePx(detailWrapper.style.top);
      const leftValue = removePx(detailWrapper.style.left);
      ...
      if (roundSize(leftValue + detailWidth) === scaleWidth) {
        console.log('соприкасается справа');
        const detailFromOriginalList = listsArray[listI].details[detail.number];
        detailFromOriginalList.touches.right += 1;
      }

      if (roundSize(topValue + detailHeight) === scaleHeight) {
        console.log('соприкасается снизу');
        const detailFromOriginalList = listsArray[listI].details[detail.number];
      ...
      if (
        roundSize(relativeY + detailHeight) > scaleHeight + scaleAppr ||
        roundSize(relativeX + detailWidth) > scaleWidth + scaleAppr ||
        roundSize(topValue + detailHeight) > scaleHeight + scaleAppr ||
        roundSize(leftValue + detailWidth) > scaleWidth + scaleAppr
      ) {
        const detailFromOriginalList = listsArray[listI].details[detail.number];
...
function sortDetails(detailsWithQuantityFill, orientation = 'row') {
  ...
  const rows = createRows(remainingDetails, sizes, orientation);
  return rows;
}
...
function createRows(remainingDetails, sizes, orientation) {
  const rows = [];
...
function renderActualListCut(hideWidth = false, renderForPDF = false) {
  ...
  if (renderForPDF) {
    ...
    console.log(listSizes, listSizeWidths, listSizeHeights);
  }
...
async function generatePDF() {
  ...
  for (const [i, list] of listsArray.entries()) {
    ...
    console.log('width', listWidth);
    console.log('height', listHeight);
    console.log('scale', listScale);
