$(document).ready(function () {
                ...
                if (data.IS_OK == "Y") {
                    ...
                    if (data.arFavoritesList.length > 0) {
                    	if(window.screen.width >= 1023){
                        	$('#js_top_favorites').show().text(data.arFavoritesList.length);
						}else if(window.screen.width < 1023){
                        	$('#js_top_favorites_mob').show().text(data.arFavoritesList.length);
                        	$('#js_top_favorites_mobpl').show().text(' ('+data.arFavoritesList.length+')');
						}
                    }else{
                    	$('#js_top_favorites').hide();
                    	$('#js_top_favorites_mobpl').hide();
                    	$('#js_top_favorites_mob').hide();
                    }
                    ...
                    if (data.arCompareList.length > 0) {
                        if(window.screen.width >= 1023){
                        	$('#js_top_compare').show().text(data.arCompareList.length);
						}else if(window.screen.width <= 1023){
							$('#js_top_compare_mob').show().text(data.arCompareList.length);
                        	$('#js_top_compare_mobpl').show().text(' ('+data.arCompareList.length+')');
						}
                    }else{
                    	$('#js_top_compare').hide();
                    	$('#js_top_compare_mobpl').hide();
                    	$('#js_top_compare_mob').hide();
                    }
    ...
    $(document).on('click','.js_togle_compare',function (e) {
        e.preventDefault();
        var t = $(this);
    ...
    $(document).on('click','.js_togle_favorite', function (e) {
        e.preventDefault();
        var t = $(this);
