...
$(document).on('click', '.header-nav-is-open .catalog-menu .catalog-menu-item-has-children>a', function (event) {
  ...
  function closeMenu() {
    ...
    li.parent().children('.catalog-menu-item-has-children').children('.catalog-submenu').fadeOut(400, function () {
      $('.header-nav .container').stop().animate({
  ...
  if (video) {
    t.on('init reInit', function (event, slick) {
		if(slick.slideCount == 1){
				console.log(t);
			   t.append('<ul class="slick-dots"><li><a href="' + video + '" target="_blank" class="item-video-link"><svg><use xlink:href="#icon-video"></use></svg></a></li></ul>');
				return;
		}
      t.find('.slick-dots').append('<li><a href="' + video + '" target="_blank" class="item-video-link"><svg><use xlink:href="#icon-video"></use></svg></a></li>');
    });
  }
...
$(".item-basket-btn").on('click',function(){
...
$(".multiplicity").change(function() {
  ...
  if ($(this).is(':checked')){
    ...
    /*if(newVal > quantityMax)
      newVal = quantityMax;*/
    $(this).parent().parent().children(".select-number-input").val(newVal + " " + edenica);
...
$('.range-slider').each(function () {
  ...
  t.slider({
    ...
    create: function create() {
      t.find('.ui-slider-handle:first').addClass('ui-slider-handle-from').append('<span class="ui-slider-handle-info">от ' + formatPrice(valueFrom) + '</span>');
      t.find('.ui-slider-handle:last').addClass('ui-slider-handle-to').append('<span class="ui-slider-handle-info">до ' + formatPrice(valueTo) + '</span>');
...
$(document).on('input', '.payment-card-input', function () {
  ...
  if (input.val().length == +input.attr('max')) {
    input.closest('.payment-card-section').find('.payment-card-input').each(function () {
      var t = $(this);
	...
	$.getScript('https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.60/pdfmake.min.js', () => {
		$.getScript('https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.20/vfs_fonts.js', () => {
			...
			$('.product-specifications-table tr').each(function(){
				...
				specifications.push(
					[
						{text: t.find('td:first-child').text().trim()},
						{text: t.find('td:last-child').text().trim()}
					]
				)
			})
...
$(document).on('click', 'a', function () {
	...
	if($(this).attr('href').match(/^\/personal\/cart\/$/)){
	...
	if($(this).attr('href').match(/^\/personal\/order\/make\/$/)){
...
(function () {
  ...
  $('.header-link[href^="mailto"]').attr('href', 'mailto:' + email).find('span').text(email);
  $('.contact-block-data [href^="mailto"]').attr('href', 'mailto:' + email).text(email);
  $('.footer-link[href^="mailto"]').attr('href', 'mailto:' + email).text(email);
