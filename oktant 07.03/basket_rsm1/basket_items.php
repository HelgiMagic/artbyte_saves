...
<table class="basket-table" id="basket_items">
    ...
    <tbody>
        ...
        <? if ($normalCount >= 0): ?>
            <? foreach ($arResult["GRID"]["ROWS"] as $k => $arItem): ?>
                ...
                <?
                    $img = $photoManager->getProductMainPhoto($arItem['PRODUCT_ID']);
                ?>
                 <?
                $allcount = 0;
                $stores = array();
                ...
                <tr class="js_basket_item"
                    ...
                    <?
                    $db_res1 = CPrice::GetList(array(),array("PRODUCT_ID"=>$arItem["PRODUCT_ID"],"CATALOG_GROUP_ID"=>8));
					...
					if (!empty($price1)) {
						if ($price1['CURRENCY']=='RUB') {
							...
							if($arDiscounts !== false) {
								...
								$currentprice = CCurrencyRates::ConvertCurrency($currentprice,$price1['CURRENCY'],"RUB");
							}else{
					...
					$arrpricecur = explode('.', number_format($currentprice, 2, '.', ' '));
                	$curpriceround = $arrpricecur[0];
                	$curpricecop = $arrpricecur[1];
                    ?>
                        <div class="item-price-current" >
                            <span id="current_price_<?=$arItem['ID'];?>"> <?=$curpriceround;?><?if(!empty($curpricecop)):?> <sup><?=$curpricecop;?></sup> <?endif;?><?//=$arItem['PRICE_FORMATED'];?></span> <span style="font-weight:200">&#8381;</span>
                        </div>
						<? //if ($arItem['PRICE_FORMATED'] < $arItem['PRICE_OLD_FORMATED'])  {
						if ($currentprice < $oldprice)  {
							$arrpriceold = explode('.', number_format($oldprice, 2, '.', ' '));
                        ...
                        <div class="item-price-old">
							<span id="old_price_<?=$arItem['ID'];?> "><?=$oldpriceround;?><?if(!empty($oldpricecop)):?> <sup><?=$oldpricecop;?></sup> <?endif;?><?//=$arItem['PRICE_OLD_FORMATED'];?></span> <span style="font-weight:200">&#8381;</span>
                        </div>

                            <? if ($skidka): ?>
                                <div class="item-price-discount item-price-stok-info">
                                <? if($USER->IsAuthorized()) { ?>
									<div class="isauth">Скидка <?//=$arItem['SKIDKA'];?><?=$skidka?>%</div>
								<? }else{ ?>
									<div class="noauth">Скидка <?//=$arItem['SKIDKA'];?><?=$skidka?>%</div>
								<? } ?>
                                </div>
						<? endif;
						}?>
                    ...
                    <td data-title="Сумма" class="sum_column">
                        <div class="basket-sum">
                            <div class="item-price-current" >
                            	<?
                            	$sumcol = $currentprice*$arItem["QUANTITY"];
                            	$arrpricecur2 = explode('.', number_format($sumcol, 2, '.', ' '));
                				$curpriceround2 = $arrpricecur2[0];
                				$curpricecop2 = $arrpricecur2[1];
                            	?>
                                <span id="sum_<?=$arItem['ID'];?>"> <?=$curpriceround2;?><?if(!empty($curpricecop2)):?> <sup><?=$curpricecop2;?></sup> <?endif;?> <?//= RSMTools::price_format($arItem['SUM_VALUE']);?> </span> <span style="font-weight:200">&#8381;</span>
                            </div>
                        </div>
                    </td>
    ...
    <tfoot>
        <tr>
			...
			<td><form method="POST" action="" style="display: inline-block;" id="clear-cart-form">
    <input type="hidden" name="BasketDelete" value="1">
    <a href="#" class="basket-link basket-link-del" onclick="confirmClearCart(); return false;">
        <i>
            <svg width="10" height="13">
                <use xlink:href="#icon-del"></use>
            </svg>
        </i>
        <span id="clear-cart-text">Очистить корзину</span>
    </a>
</form>

<script>
    function confirmClearCart() {
        var confirmation = confirm("Вы уверены, что хотите очистить корзину?");
        var clearCartText = document.getElementById("clear-cart-text");

        if (confirmation) {
            // Если пользователь подтвердил, отправляем форму
            document.getElementById("clear-cart-form").submit();
        } else {
            // Если нет, восстанавливаем первоначальный текст
            clearCartText.innerHTML = "Очистить корзину";
        }
    }
</script>
            </td>
                    ...
                    <div class="basket-total-body">
                        <div class="basket-total">
                            <div class="basket-total-line basket-total-sum">
                                ...
                                <?
                				$arResult["allSum"] = $allsumm;
                				?>
                                    <div class="item-price-current"> <span id="allSum"> <?= RSMTools::price_format($arResult["allSum"]);?></span> <span style="font-weight:200">&#8381;</span></div>
                                </div>
                            </div>
                        </div>
