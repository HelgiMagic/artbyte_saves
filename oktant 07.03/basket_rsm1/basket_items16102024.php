        ...
        <? if ($normalCount >= 0): ?>
            <? foreach ($arResult["GRID"]["ROWS"] as $k => $arItem): ?>
                ...
                <tr class="js_basket_item"
					...
					$arrpricecur = explode('.', $currentprice);
                	$curpriceround = $arrpricecur[0];
                	$curpricecop = $arrpricecur[1];
                    ?>
                        <div class="item-price-current" >
                            <span id="current_price_<?=$arItem['ID'];?>"> <?=$curpriceround;?><?if(!empty($curpricecop)):?> <sup><?=$curpricecop;?></sup> <?endif;?><?//=$arItem['PRICE_FORMATED'];?></span> <span class="rouble">q</span>
                        </div>
						<? //if ($arItem['PRICE_FORMATED'] < $arItem['PRICE_OLD_FORMATED'])  {
						if ($currentprice < $oldprice)  {
							$arrpriceold = explode('.',$oldprice);
							$oldpriceround = $arrpriceold[0];
							$oldpricecop = $arrpriceold[1];
						?>
                        <div class="item-price-old">
							<span id="old_price_<?=$arItem['ID'];?> "><?=$oldpriceround;?><?if(!empty($oldpricecop)):?> <sup><?=$oldpricecop;?></sup> <?endif;?><?//=$arItem['PRICE_OLD_FORMATED'];?></span> <span class="rouble">o</span>
                        </div>
                    ...
                    <td data-title="Сумма" class="sum_column">
                        <div class="basket-sum">
                            <div class="item-price-current" >
                            	...
                            	$arrpricecur2 = explode('.', $sumcol);
                				$curpriceround2 = $arrpricecur2[0];
                				$curpricecop2 = $arrpricecur2[1];
                            	?>
                                <span id="sum_<?=$arItem['ID'];?>"> <?=$curpriceround2;?><?if(!empty($curpricecop2)):?> <sup><?=$curpricecop2;?></sup> <?endif;?> <?//= RSMTools::price_format($arItem['SUM_VALUE']);?> </span> <span class="rouble">q</span>
                            </div>
                        </div>
                    </td>
