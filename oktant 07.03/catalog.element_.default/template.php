...
$page_url = 'https://' . $_SERVER["SERVER_NAME"] . $APPLICATION->GetCurPage();
$APPLICATION->SetPageProperty('canonical', $page_url);
...
//иконки
$stock = $arResult['PROPERTIES']['TSENOVAYA_GRUPPA']['VALUE'];
...
$currentprice_clean = (float)str_replace(' ', '', $currentprice) ;
if (!empty($price2)) {
...
foreach ($arResult['DISPLAY_PROPERTIES'] as $p) {
    ...
    if (!empty($pval)):
        if($p['ID'] == '475' && !empty($p['VALUE'])){
            ...
            $showprps[$p['NAME']]['NAME'] = $pval;
            $showprps[$p['NAME']]['HINT'] = $p['HINT'];
        }
    endif;
}
//описание 
...
function array_multisort_value()
	...
	function my_compare($a, $b) {
	  if ($a['IBLOCK_SECTION_ID'] == $b['IBLOCK_SECTION_ID'])
	    ...
	    return $a['IBLOCK_SECTION_ID'] - $b['IBLOCK_SECTION_ID'];
	}
...
if (!empty($complect)) {
    ...
    $res_complects = CIBlockElement::GetList(array("IBLOCK_SECTION_ID" => "ASC"), $arFilter_complects);
    $contComlects = "";
    ...
    while ($ob_complects = $res_complects->GetNextElement()) {
        ...
        if($ddd == 1){
    		$arFields_complects4sect = $arFields_complects['IBLOCK_SECTION_ID'];
		}
        ...
        if ($arFields_complects['IBLOCK_SECTION_ID'] > 0):
			...
			$resStore = CCatalogStore::GetList(
			   ...
			   array('ACTIVE'=>'Y','PRODUCT_ID'=>$arFields_complects['ID']),
			   false,
			   false,
			   array()
			);
			while($sklad = $resStore->Fetch()){
					unset($AMOUNT);
					$arFilter_store = array("PRODUCT_ID"=>$arFields_complects['ID'],"STORE_ID"=>$sklad['ID']);
					...
					//echo $AMOUNT;
					$arFields_complects['QUANTITY'] = $allcount;
			}
        	//$activeElementsCount = CIBlockSection::GetSectionElementsCount($arFields_complects['IBLOCK_SECTION_ID'], Array("CNT_ACTIVE"=>"Y"));
        	//echo $arFields_complects4sect;
        	if($arFields_complects4sect == $arFields_complects['IBLOCK_SECTION_ID']){
            	$array_complects[$arFields_complects['IBLOCK_SECTION_ID']]['name'] = $arFields_complects['IBLOCK_SECTION_ID'];
            	$array_complects[$arFields_complects['IBLOCK_SECTION_ID']]['count'] = $ccc;
			}else{
				$ccc = 1;
				$array_complects[$arFields_complects['IBLOCK_SECTION_ID']]['name'] = $arFields_complects['IBLOCK_SECTION_ID'];
            	$array_complects[$arFields_complects['IBLOCK_SECTION_ID']]['count'] = $ccc;
            ...
            $ddd++;
        	/*if($arFields_complects4sect == $arFields_complects['IBLOCK_SECTION_ID']){
            	$array_complects[]['name'] = $arFields_complects['IBLOCK_SECTION_ID'];
            	$array_complects[]['count'] = $ccc;
        ...
        $array_complects2[] = $arFields_complects;
        $contComlects++;
    }
}

$array_complects = array_multisort_value($array_complects, 'count', SORT_DESC);
$array_complects3 = [];
?>
<div style="display: none;">
	<?
	...
	if(!empty($array_complects2)){
	
	$array_complects3 = array_multisort_value($array_complects2, 'QUANTITY', SORT_DESC);
	

	foreach ($array_complects3 as $comps3):
		if($comps3['QUANTITY'] > 0){
			foreach($array_complects as $prodp){
				print_r($prodp);
				if($prodp['name'] == $comps3['IBLOCK_SECTION_ID']){
					$comps3['PRODCOUNT'] = $prodp['count'];
				}
			}
			$array_complects4[] = $comps3;
			$array_sect4[] = $comps3['IBLOCK_SECTION_ID'];
		}else{
			$array_complects5[] = $comps3;
			$array_sect5[] = $comps3['IBLOCK_SECTION_ID'];
		}
	endforeach;
	
	$array_complects4 = array_multisort_value($array_complects4, 'IBLOCK_SECTION_ID', SORT_ASC, 'QUANTITY', SORT_DESC);
	//usort($array_complects4, "my_compare");
	$array_complects5 = array_multisort_value($array_complects5, 'IBLOCK_SECTION_ID', SORT_ASC);
	$array_sect4 = array_unique($array_sect4);
	//$array_sect4 = sort($array_sect4);
	$count_array_sect4 = count($array_sect4);
	$array_sect5 = array_unique($array_sect5);
	//$array_sect5 = sort($array_sect5);
	$count_array_sect5 = count($array_sect5);
	
	$comps4sect = 0;
	$sectii = 1;
	//for($ii=1; $ii<=$count_array_sect4; $ii++){
		foreach($array_complects4 as $comps4) {
			if($comps4sect == $comps4['IBLOCK_SECTION_ID']){
				$comps4['ORDER'] = $sectii;
			}else{
				$sectii = 1;
				$comps4['ORDER'] = $sectii;
			...
			$sectii++;
			$array_complects44[] = $comps4;
			$comps4sect = $comps4['IBLOCK_SECTION_ID'];
		}
	//}
	
	$array_complects44 = array_multisort_value($array_complects44, 'ORDER', SORT_ASC, 'PRODCOUNT', SORT_DESC);
	
	$comps5sect = 0;
	$sectii2 = 1;
	//for($ii=1; $ii<=$count_array_sect4; $ii++){
		foreach($array_complects5 as $comps5) {
			if($comps5sect == $comps5['IBLOCK_SECTION_ID']){
				$comps5['ORDER'] = $sectii2;
			}else{
				$sectii2 = 1;
				$comps5['ORDER'] = $sectii2;
			}
			$sectii2++;
			$array_complects55[] = $comps5;
			$comps5sect = $comps5['IBLOCK_SECTION_ID'];
		}
	//}
	
	$array_complects55 = array_multisort_value($array_complects55, 'ORDER', SORT_ASC);
	
	/*$values2 = array_values($array_sect4);

	foreach($array_complects4 as $key => $val) {
	    
	    $idx2 = $key % count($array_sect4);
		$array_complects44[] = $val;
	    echo $val['ID'].' - '. $values2[$idx2]."\n";
	}*/

	//print_r($array_filter4);
	//print_r($array_complects44);
...
//var_dump($arResult);
$jsonData = json_encode($complect);
?>

<script type="text/javascript">
$(document).on('click', '.tabs-nav-komplect a', function (event) {
	event.preventDefault();
	//$('.descmore2').hide();
	let sectid = $(this).attr('data-sect');
	//var t = $(this),
      			//li = t.parent(),
      			//tab = t.attr('href'),
      			//tabs = t.closest('.tabs-nav').data('tabs');
	$.ajax({
            url: '/bitrix/templates/.default/components/bitrix/catalog/newoktant/bitrix/catalog.element/.default/ajax/load_products.php',  // Куда отправить запрос
            method: 'POST',                  // Метод запроса (POST или GET)
            dataType: '',
            data: {data: <?=$jsonData?>, sect: sectid},          // Данные, передаваемые в массиве
            success: function(data) {        // Функция, которая будет выполнена после успешного запроса
                console.log(data);           // В переменной data содержится ответ от load_products.php
                // Здесь вы можете добавить код для обновления DOM с новыми данными
                
                $('.items-slider-komplect-more').html(data);
...
<div <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemscope itemtype="https://schema.org/Product"<? endif; ?>>
    ...
    <div class="product" data-id="<?= $arResult['ID']; ?>">
        ...
        <div class="product-gallery">
            <div class="product-main-slider">
                <?
				$cn = 1;
                foreach ($arResult['PHOTO_LIST'] as $f):
                    
                    ?>
                    <div class="product-main-slide">
                        <div class="product-main-img">
                            <a href="<?= $f; ?>" class="glightbox"><img <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>itemprop="image"<? endif; ?> src="<?= $f; ?>" alt="<?= $name; ?> <?= $tovcod; ?> <?= $cn; ?>" title="<?= $name; ?> <?= $tovcod; ?> <?= $cn; ?>"></a>
                        </div>
                    </div>
                    <?
                    $cn++;
                        ...
                        foreach ($arResult['PHOTO_LIST'] as $f):
                            ...
                            <div class="product-nav-slide">
                                <div class="product-nav-slide-img">
                                    <img src="<?= $f; ?>" alt="<?= $name; ?> <?= $tovcod; ?> <?= $cn; ?>" title="<?= $name; ?> <?= $tovcod; ?> <?= $cn; ?>">
                                </div>
                            </div>
                            <?
                            $cn++;
        ...
        <div class="product-body">
            <h1 class="section-title desktop-only" <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>itemprop="name"<? endif; ?>><?= $arResult["NAME"]; ?></h1>
                ...
                <div class="product-content-row flex-row">
                    <div class="product-content-col flex-row-item">
                        ...
                        <div class="item-details">
                            <? if ($tovcod): ?>
                                <div class="product-code" <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>itemprop="model"<? endif; ?>>Артикул: <?= $tovcod; ?></div>
                            <? endif; ?>
                            <? if ($brandarr): ?>
                                            ...
                                            if (is_array($sectionProperty) && count($sectionProperty) > 0) {
                                                foreach ($sectionProperty as $item) {
                                                    ...
                                                    if ($item["ACTIVE"] != "N") {
                                                        ...
                                                        $arSmartFilterPropsHints[$item["PROPERTY_CODE"]] = $item["PROPERTY_HINT"];
                                            ...
                                            foreach ($arSmartFilterProps as $key => $value) {
                                                ...
                                                if ($value) {
                                                    ...
                                                    if ($propName) {
														...
														$propsString .= '<tr><td><div class="product-info-table-title"><span>'.$propName.'</span></div></td><td><span>'.$val.' '.$arSmartFilterPropsHints[$value].'</span>';
                                                    ...
                                                    if (is_array($elemsNew) && count($elemsNew) > 0) {
                                                        ...
                                                        foreach ($elemsNew as $elemNew) {
                                                            ...
                                                            if (array_key_exists($propName, $arSelectedProps) && array_search($elemNew[$value.'_VALUE'], $arSelectedProps[$propName])) {//&& $arSelectedProps[$propName][$elem[$value.'_VALUE']]
                                                                ...
                                                                $arPropsString[$loopProp] = '<a href="'.$elemNew['DETAIL_PAGE_URL'].'">'.$loopProp.' '.$arSmartFilterPropsHints[$value].'</a>';
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <?if(!empty($arOneProp["URL"])):?>
                                                                    <a href="<?=$arOneProp["URL"]?>" target="_blank"><?=$pval;?> <?= $arOneProp['HINT']; ?></a>
                                                                <?else:?>
                                                                    <span><?= $pval; ?> <?= $arOneProp['HINT']; ?></span>
                                                                <?endif;?>
                                                            </td>
                                                        </tr>
                                                        <?php
                    ...
                    <li>
                        <a href="#" class="product-print-link">
                            ...
                            <div class="product-form-body">
                                ...
                                <div class="item-prices prodelement" <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>itemprop="offers" itemscope="itemscope" itemtype="https://schema.org/Offer"<? endif; ?>>
                                    <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?><?if($allcount>0):?>
                                        <link itemprop="availability" href="https://schema.org/InStock">
                                        <meta itemprop="sku" content="<?=$allcount?>" />
                                    <?endif;?><? endif; ?>

                                    <?/* Старый вариант старой цены
                                    //if ((!empty($oldprice))AND ( $currentprice != $oldprice)):
                                        ...
                                        //
                                        ?>
                                        <div class="item-price-old"><?//= $oldpriceround; ?><? //if (!empty($oldpricecop)): ?> <sup><?//= $oldpricecop; ?></sup> <? //endif; ?> <span style="font-weight:200">&#8381;</span></div>
                                    <? //endif; ?>
                                    <? //if ($skidka): ?>
                                    ...
                                    */ ?>
											...
											$arrpricecur = explode('.', $price['PRINT_RATIO_PRICE']);
                                        	$curpriceround = $arrpricecur[0];
                                        	$curpricecop = $arrpricecur[1];
											$currentprice_clean = (float)str_replace(' ', '', $curpriceround) . '.'.$curpricecop ;
											?>
											
											<div class="item-price-current" id="<?=$arItemIDs['PRICE']?>"><?= $curpriceround; ?><? if (!empty($curpricecop)): ?> <sup><?= $curpricecop; ?></sup> <span style="font-weight:200">&#8381;</span><? endif; ?></div>
                                        	<?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>
												<meta itemprop="price" content="<?= !empty($price['PRINT_RATIO_PRICE']) ? $currentprice_clean  : 'По запросу' ?>">
											...
											if ($arParams['SHOW_OLD_PRICE'] === 'Y')
											{
												?>
													...
													if ($showDiscount)
													{
														//echo $price['PRINT_RATIO_DISCOUNT'];
													}
													 ?>
											...
											if ($arParams['SHOW_OLD_PRICE'] === 'Y')
											{
												if ($showDiscount)
												{
													$arrpriceold = explode('.', $price['PRINT_RATIO_BASE_PRICE']);
												...
												<div>
												<? if ($stock == "SALE") { ?>
													<div style="display: <?=($showDiscount ? '' : 'none')?>;" class="item-price-stok-info sale">–<?= $price['PERCENT']; ?>%</div>
												<? } else { ?>
													<? if ($USER->IsAuthorized()) { ?>
														<div style="display: <?=($showDiscount ? '' : 'none')?>;" class="item-price-stok-info isauth">–<?= $price['PERCENT']; ?>%</div>
													<? } else { ?>
														<div style="display: <?=($showDiscount ? '' : 'none')?>;" class="item-price-stok-info noauth">–<?= $price['PERCENT']; ?>%</div>
													<? } ?>
												<? } ?>
													<div class="item-price-old" id="<?=$arItemIDs['OLD_PRICE']?>" style="display: <?=($showDiscount ? '' : 'none')?>;">
														<?= $oldpriceround; ?><? if (!empty($oldpricecop)): ?> <sup><?= $oldpricecop; ?></sup> <? endif; ?> <span style="font-weight:200">&#8381;</span>
													</div>
                                    ...
                                    <?/* Старый вариант актуальной цены
                                    ...
                                    if (!empty($currentprice)):
                                        ...
                                        ?>
                                        <div class="item-price-current"><?= $curpriceround; ?><? if (!empty($curpricecop)): ?> <sup><?= $curpricecop; ?></sup> <? endif; ?> <span style="font-weight:200">&#8381;</span></div>
                                        <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>
											<meta itemprop="price" content="<?= $currentprice_clean ?>" dd>
											<meta itemprop="priceCurrency" content="RUB">
										<? endif; ?>	
                                     endif; */?>
							...
							<div style="margin: 18px 0 0 0; display: flex; justify-content: space-between; align-items: center;">
							<?
							/*\Bitrix\Main\Loader::includeModule('bxmaker.geoip');
							$oManager = \Bxmaker\GeoIP\Manager::getInstance();
							$citycurrent = $oManager->getPosition()?->getItem()?->getName();*/
			...
			//if (CModule::IncludeModule('edost.locations')) $GLOBALS['APPLICATION']->IncludeComponent('edost:locations', '', array('MODE' => 'header'), null, array('HIDE_ICONS' => 'Y'));
								$edostclass = new edost_class();
								$textedost = $edostclass->edost_calc($citycurrent, 0.1, '', '', '', '');
								$arrdelivery = [];
							...
							if(isset($_COOKIE['deliverys'])){
								...
								$arrdelivery = stripslashes($_COOKIE["deliverys"]);
								$arrdelivery = json_decode($arrdelivery, JSON_OBJECT_AS_ARRAY);
								
								$arrdeliverycity = stripslashes($_COOKIE["deliverycity"]);
								$arrdeliverycity = json_decode($arrdeliverycity, JSON_OBJECT_AS_ARRAY);
								
								$arrdeliveryqty = stripslashes($_COOKIE["deliveryqty"]);
								$arrdeliveryqty = json_decode($arrdeliveryqty, JSON_OBJECT_AS_ARRAY);
								
								
								foreach($arrdelivery as $arrd){
									if($arrd['companyfull'] == $_COOKIE["deliveryname"]){
										if(!empty($arrd['id']) and !empty($arrd['day'])  and $arrd['day']!="&nbsp;" and $arrd['price'] > 0){
											
											$srok = explode(' ', $arrd['day']);
											
											$srokdays = explode('-', $srok[0]);
										...
										if($_COOKIE["deliveryname"] == 'Самовывоз '){
												$srokf = 0;
											}else{
												$srokf = 2;
											}
									...
									if(empty($srokf)){
										if(!empty($arrdelivery[0]['id']) and !empty($arrdelivery[0]['day'])  and $arrdelivery[0]['day']!="&nbsp;" and $arrdelivery[0]['price'] > 0){
											
											$srok = explode(' ', $arrdelivery[0]['day']);
							...
							if(!empty($srokf)){
								if(!empty($arResult["PROPERTIES"]["SROK_POSTAVKI"]["VALUE"])){
                            		$srok_postavki = $arResult["PROPERTIES"]["SROK_POSTAVKI"]["VALUE"]+$srokf;
								}
								if(!empty($arResult["PROPERTIES"]["EKSPRESS_DOSTAVKA"]["VALUE"])){
                            		$ekspress_dost = $arResult["PROPERTIES"]["EKSPRESS_DOSTAVKA"]["VALUE"]+$srokf;
								}
								if(!empty($arResult["PROPERTIES"]["STANDARTNAYA_DOSTAVKA"]["VALUE"])){
                            		$standartnaya_dost = $arResult["PROPERTIES"]["STANDARTNAYA_DOSTAVKA"]["VALUE"]+$srokf;
								}
							}else{
							...
							$checkd = 0;
							//echo $_COOKIE["deliveryname"];
							if(in_multi_array($_COOKIE["deliveryname"], $arrdelivery)){ 
								$checkd = 1;
							} 
							//echo $checkd;
							//print_r($arrdelivery);
							...
							$(document).ready(function(){
								var deliveryname2 = getCookie('deliveryname');
								console.log(deliveryname2);
								if(deliveryname2 !== undefined){
									let picdeliv = "";
									if(deliveryname2 == "Самовывоз "){
										picdeliv = "/local/templates/oktant/img/samov.svg";
									}else if(deliveryname2 == "Деловые линии до терминала" || deliveryname2 == "Деловые линии до подъезда"){
										picdeliv = "/local/templates/oktant/img/dl1.svg";
									}else if(deliveryname2 == "СДЭК посылка - до пункта выдачи" || deliveryname2 == "СДЭК посылка - курьером до двери"){
										picdeliv = "/local/templates/oktant/img/cdek.svg";
									}
									
									$('.delivery-name-edost-img').html('<img src="'+picdeliv+'" width="60" height="32" border="0">');
									
								}else{
									
									let picdeliv = "";
									if($('.modal-delivery-text.active .modal-delivery-name').attr('data-company') == "Самовывоз "){
										picdeliv = "/local/templates/oktant/img/samov.svg";
									}else if($('.modal-delivery-text.active .modal-delivery-name').attr('data-company') == "Деловые линии до терминала" || deliveryname2 == "Деловые линии до подъезда"){
										picdeliv = "/local/templates/oktant/img/dl1.svg";
									}else if($('.modal-delivery-text.active .modal-delivery-name').attr('data-company') == "СДЭК посылка - до пункта выдачи" || deliveryname2 == "СДЭК посылка - курьером до двери"){
										picdeliv = "/local/templates/oktant/img/cdek.svg";
									}
							...
							if($arrdelivery[0]['id'] == 35){
								$picdeliv = "/local/templates/oktant/img/samov.svg";
							}elseif($arrdelivery[0]['id'] == 22 or $arrdelivery[0]['id'] == 51){
								$picdeliv = "/local/templates/oktant/img/dl1.svg";
							}elseif($arrdelivery[0]['id'] == 37 or $arrdelivery[0]['id'] == 38){
								$picdeliv = "/local/templates/oktant/img/cdek.svg";
							}else{
								$picdeliv = "/edost/delivery_img/".$arrdelivery[0]['id'].".gif";
							}
							...
							?>
								...
								$(document).ready(function(){
									$('.delivery-name-edost-img').html('<img src="<?=$picdeliv?>" width="60" height="32" border="0">');
									$('.delivery-name-edost').html("<?=$arrdelivery[0]['companyfull']?>");
								});
								</script>
							
							<? } ?>
							
							<div style="" class="delivery-name-edost-img"></div>
                            ...
                            <div class="avail">
                                <div class="avail-instock">
									...
									<?php if ($arResult['PROPERTIES']['CML2_MANUFACTURER']['VALUE'] == 'KLEMSAN') {
                                        $storeId = 9;
                                        $rsStore = StoreProductTable::getList(array(
                                            'filter' => array('=PRODUCT_ID'=>$arResult['ID'], 'STORE_ID' => $storeId),
                                            'select' => array('AMOUNT'),
                                    ...
                                    <?if(!empty($allcount)):?>
                                        <section>
                                        	...
                                        	if($srokf>0){
                                        		$now0 = time();
                                        		$deliveryTime0 = $now0 + (int)$srokf*24*60*60;
                                                $deliveryDate0 = date('d.m.y', $deliveryTime0);
                                        	?>
                                        	<p><?=$deliveryDate0;?> (<?=$srokf?> <?= getNoun((int)$srokf, 'день', 'дня', 'дней');?>)</p><p><?=number_format($allcount,0,'',' ');?> <?=$edenica;?></p>
                                        	<?
											}else{
											?>
                                                ...
                                                if ($v['unixtime'] >= time() && $k >= 0) {?>
                                                    ...
                                                    <section>
                                                        <p>
                                                            ...
                                                            <?php 
                                                            if(!empty($arrdelivery[0]['srok'])){
                                                            	$k = $k+1+$arrdelivery[0]['srok'];
                                                            }else{
                                                            	$k = $k+1;
															}
                                                            endif;?>
    ...
    <div class="product-tabs">
        <ol class="product-tabs-nav">
            ...
            <li target="analogs"><button <?if(count($array_analogs) == 0):?>disabled<?endif?>><span>АНАЛОГИ <i><?=count($array_analogs)?></i></span></button></li>
        ...
        <div class="product-tabs-content">
            <section class="current" tabname="description">
                <div class="tab-trigger"><button><span>Описание</span></button></div>
                <div class="tab-content">
                    <? //opisanie i harakteristiki ;?>
                    <? /*<section>
                            <div class="content-text product-desc" <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="description" <? endif; ?>>
                            </div>
                    </section> */ ?>
                    
                    <!--новые блоки-->
                    <? if ($opisanie): ?>
                    <section id="product-specifications" class="opisanieblocks">
                                ...
                                <div class="product-specifications-col">
                                    <table class="product-info-table product-specifications-table">
                                        ...
                                        <? foreach ($proptable1 as $n => $p) { ?>
                                            ...
                                            <tr <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="additionalProperty" itemscope="itemscope" itemtype="https://schema.org/PropertyValue"<? endif; ?>>
                                                <td>
                                                    <div class="product-info-table-title"><span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>itemprop="name"<? endif; ?>><?= $n; ?></span></div>
                                                </td>
                                                <td class="proprightvalue">
                                                    <?if(!empty($p["URL"])):?>
                                                        <a href="<?=$p["URL"]["SRC"]?>" target="_blank"><span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="value"<? endif; ?>><?=$p["URL"]["NAME"]?></span></a>
                                                    <?else:?>
                                                        <span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?>itemprop="value"<? endif; ?>><?=$p["NAME"];?>  <?=$p["HINT"]?></span>
                                                    <?endif;?>
                                                </td>
                                            </tr>
                                        <? } ?>
            ...
            <section class="current" tabname="documents">
            <div class="tab-trigger"><button><span>Документация</span></button></div>
            <div class="opisanieblocks twoblocksinner product-specifications-row">
                            <div class="product-specifications-col docsblock">
                            <? if (CModule::IncludeModule('iblock')) :  ?>
                            <?
									$property_enumsH = CIBlockPropertyEnum::GetList(Array("DEF"=>"DESC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>26, "CODE"=>"TYPE_DOP_INFO"));
									if($property_enumsH->SelectedRowsCount() > 0) {
									while($enum_fieldsH = $property_enumsH->GetNext())
									{
										//print_r($arResult['PROPERTIES'][$enum_fieldsH["XML_ID"]]);
										
										unset($arResult['PROPERTIES'][$enum_fieldsH["XML_ID"]]['USER_TYPE_SETTINGS']);
										$arAvtSelectH = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_*");
							            $arAvtFilterH = Array("IBLOCK_ID"=>26, "ACTIVE"=>"Y", "NAME" => $arResult['PROPERTIES'][$enum_fieldsH["XML_ID"]]);
							            $resAvtH = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arAvtFilterH, false, false, $arAvtSelectH);
									}
									}
							            ?>
							    <? if(true) { ?>       
	                            <h2 class="product-info-title desktop-only">Документация</h2>
	                            <? } ?>
	                            <div class="product-specifications-row twoblocks">
									<?
									$property_enums = CIBlockPropertyEnum::GetList(Array("DEF"=>"DESC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>26, "CODE"=>"TYPE_DOP_INFO"));
									?>
										<div style="display: none;">
										<?
										print_r($property_enums->SelectedRowsCount());
										?>
										</div>
										<?
									...
									while($enum_fields = $property_enums->GetNext())
									{
										...
										$arAvtSelect = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_*");
										unset($arResult['PROPERTIES'][$enum_fields["XML_ID"]]['USER_TYPE_SETTINGS']);
										
										
							            $arAvtFilter = Array("IBLOCK_ID"=>26, "NAME" => $arResult['PROPERTIES'][$enum_fields["XML_ID"]], "PROPERTY_TYPE_DOP_INFO_VALUE"=>$enum_fields["VALUE"]);
							            $resAvt = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arAvtFilter, false, false, $arAvtSelect);
							            ?>
										<div style="display: none;">
										<?
										print_r($resAvt->SelectedRowsCount());
										?>
										</div>
										<?
								            if($resAvt->SelectedRowsCount() > 0) {
									            while($obAvt = $resAvt->GetNextElement())
							            		{
									                $arAvtFields = $obAvt->GetFields();
									                $arAvtProperties = $obAvt->GetProperties();
									                foreach($arAvtProperties["DOCS"]["VALUE"] as $docs){
										                $fileparams = CFile::GetFileArray($docs);
										                //print_r(CFile::GetFileArray($arAvtProperties["DOCS"]["VALUE"][0]));
										                if($fileparams['CONTENT_TYPE']=="application/pdf"){
										                	$urlicon = "/local/templates/oktant/img/icons/docs/pdf.svg";
										                }else{
										                	$urlicon = "/local/templates/oktant/img/icons/docs/noicon.svg";
										                }
														...
														//$size = '('.str_replace('.', ',', round($size, 1)).' Кб)';
														$size = str_replace('.', ',', formatBytes($fileparams["FILE_SIZE"]));
														
										                ?>
										                <div class="doclineblock product-specifications-col twoblocks">
															<div class="icon twoblocks"><img src="<?=$urlicon?>"/></div>
															<a href="<?=CFile::GetPath($docs)?>" target="_blank" class="text">
																<?=$fileparams["DESCRIPTION"]?> <?=$size?>
															</a>
														</div> 
									            <?	}
												}	
                            ...
                            <div class="product-specifications-col logistblock">
                            <? if(!empty($arResult['PROPERTIES']["VES"]["VALUE"]) or !empty($arResult['PROPERTIES']["OBYOM"]["VALUE"]) or !empty($arResult['PROPERTIES']["KRATNOST_PRODAZHI"]["VALUE"]) or !empty($arResult['PROPERTIES']["UPAKOVKA"]["VALUE"])){ ?>
	                            <h2 class="product-info-title">Логистические параметры</h2>
	                        <? } ?>
	                            <div class="product-specifications-row">
	                            <div class="">
                                    <table class="product-info-table product-specifications-table">
                                            ...
                                            <tr <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="additionalProperty" itemscope="itemscope" itemtype="https://schema.org/PropertyValue" <? endif; ?>>
                                                <td>
                                                    <div class="product-info-table-title"><span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="name" <? endif; ?>>Вес нетто</span></div>
                                                </td>
                                                <td class="proprightvalue">
                                                        <span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="value" <? endif; ?>><?=round($arResult['PROPERTIES']["VES"]["VALUE"], 3)?>  <?=$arResult['PROPERTIES']["VES"]["HINT"]?></span>
                                                </td>
                                            </tr>
                                            <? } ?>
                                            <? if(!empty($arResult['PROPERTIES']["OBYOM"]["VALUE"])) { ?>
                                            <tr <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="additionalProperty" itemscope="itemscope" itemtype="https://schema.org/PropertyValue"<? endif; ?>>
                                                <td>
													<div class="product-info-table-title"><span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="name"<? endif; ?>>Объём</span></div>
												</td>
												<td class="proprightvalue">
													<span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="value" <? endif; ?>>
														<?=number_format($arResult['PROPERTIES']["OBYOM"]["VALUE"], 6, ',', '')?> <?=$arResult['PROPERTIES']["OBYOM"]["HINT"]?>
													</span>
												</td>
                                            </tr>
                                            <? } ?>
                                            ...
                                            <tr <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="additionalProperty" itemscope="itemscope" itemtype="https://schema.org/PropertyValue" <? endif; ?>>
                                                <td>
                                                    <div class="product-info-table-title"><span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="name"<? endif; ?>>Кратность продажи</span></div>
                                                </td>
                                                <td class="proprightvalue">
                                                        <span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="value"<? endif; ?>><?=$arResult['PROPERTIES']["KRATNOST_PRODAZHI"]["VALUE"]?>  <?=$arResult['PROPERTIES']["KRATNOST_PRODAZHI"]["HINT"]?></span>
                                                </td>
                                            </tr>
                                            <? } ?>
                                            <? if(!empty($arResult['PROPERTIES']["UPAKOVKA"]["VALUE"])) { ?>
                                            <tr <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="additionalProperty" itemscope="itemscope" itemtype="https://schema.org/PropertyValue"<? endif; ?>>
                                                <td>
                                                    <div class="product-info-table-title"><span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="name"<? endif; ?>>Упаковка</span></div>
                                                </td>
                                                <td class="proprightvalue">
                                                        <span <?if(((!empty($currentprice))OR(!empty($oldprice)))AND(($currentprice!='0.01')OR($oldprice!='0.01'))):?> itemprop="value"<? endif; ?>><?=$arResult['PROPERTIES']["UPAKOVKA"]["VALUE"]?>  <?=$arResult['PROPERTIES']["UPAKOVKA"]["HINT"]?></span>
                                                </td>
                                            </tr>
                                            <? } ?>
                                    </table>
                         ...
                         <!--новые блоки-->
                    <section id="" class="techphotosmain">
                            <div class="product-specifications-row twoblocks opisanieblocks techphotos">
                            	<? if (CModule::IncludeModule('iblock')) :  ?>
									<?
									$property_enums = CIBlockPropertyEnum::GetList(Array("DEF"=>"DESC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>23, "CODE"=>"TYPE_DOP_INFO"));
									if($property_enums->SelectedRowsCount() > 0) {
									while($enum_fields2 = $property_enums->GetNext())
									{
										
										$arAvtSelect = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_*");
										unset($arResult['PROPERTIES'][$enum_fields2["XML_ID"]]['USER_TYPE_SETTINGS']);
							            $arAvtFilter = Array("IBLOCK_ID"=>23, "NAME" => $arResult['PROPERTIES'][$enum_fields2["XML_ID"]], "PROPERTY_TYPE_DOP_INFO_VALUE"=>$enum_fields2["VALUE"]);
							            $resAvt = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arAvtFilter, false, false, $arAvtSelect);
							            if($resAvt->SelectedRowsCount() > 0) {
							            while($obAvt = $resAvt->GetNextElement())
							            {
							                $arAvtFields = $obAvt->GetFields();
							                $arAvtProperties = $obAvt->GetProperties();
							                //print_r($arAvtProperties);
							                foreach($arAvtProperties["TECH_PHOTOS"]["VALUE"] as $photos){
							                $fileparams = CFile::GetFileArray($photos);
							                ?>
							                <div class="techphoto">
												<a class="techphoto_img twoblocks glightbox" data-gallery="gallerytech" href="<?=CFile::GetPath($photos)?>">
													<img src="<?=CFile::GetPath($photos)?>" /><?//=$enum_fields["VALUE"]?> <?//=CFile::GetFileArray($arAvtProperties["TECH_PHOTOS"]["VALUE"][0])?>
												</a>
												<div class="descph"><?if (!empty($fileparams["DESCRIPTION"])) {
													echo $fileparams["DESCRIPTION"];
													} else {
														echo $enum_fields2["VALUE"];
													}
													?></div>
											</div> 
							            <?}
                    ...
                    <!--новые блоки-->
                    <section id="" class="techvideosmain">
                            <div class="product-specifications-row opisanieblocks techvideos twoblocks">
                            	<? if (CModule::IncludeModule('iblock')) :  ?>
									<?
										$arAvtSelect = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_*");
										unset($arResult['PROPERTIES']['VIDEO']['USER_TYPE_SETTINGS']);
							            $arAvtFilter = Array("IBLOCK_ID"=>25, "NAME" => $arResult['PROPERTIES']['VIDEO']);
							            $resAvt = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arAvtFilter, false, false, $arAvtSelect);
							            if($resAvt->SelectedRowsCount() > 0) {
							            while($obAvt = $resAvt->GetNextElement())
							            {
							                ...
							                ?>
							                <div class="">
												<div class="techvideo twoblocks">
													<div>
														<iframe width="465" height="257" src="https://rutube.ru/play/embed/<?=$arAvtProperties["RUTUBE"]["VALUE"][0]?>" frameBorder="0" allow="clipboard-write; autoplay" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
														<div class="descph"><?=$arAvtProperties["RUTUBE"]["DESCRIPTION"][0]?></div>
													</div>
												</div>
											</div> 
							            <?}
                    ...
                    <!--новые блоки-->
                    <section id="" class="opisanieserii">
                            <div class="product-specifications-row opisanieblocks">
                            	<? if (CModule::IncludeModule('iblock')) :  ?>
									<?
										$arAvtSelect = Array("ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT", "PROPERTY_*");
										unset($arResult['PROPERTIES']['OPISANIE_SERII']['USER_TYPE_SETTINGS']);
							            $arAvtFilter = Array("IBLOCK_ID"=>24, "NAME" => $arResult['PROPERTIES']['OPISANIE_SERII']);
							            $resAvt = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arAvtFilter, false, false, $arAvtSelect);
							            if($resAvt->SelectedRowsCount() > 0) {
							            while($obAvt = $resAvt->GetNextElement())
							            {
							                ...
							                ?>
							                <div class="avText">
												<?=$arAvtFields["PREVIEW_TEXT"]?>
											</div> 
											<div class="descmore">Показать всё</div>
							            <?}
										}
							            ?>
								<? endif ?>
            ...
            <? if ($contComlects > 0): ?>
            <section class="current" tabname="components">
                <div class="tab-trigger"><button><span>Комплектующие <i><?=$contComlects?></i></span></button></div>
                <div class="tab-content">
                    <h2 class="product-info-title desktop-only">Комплектующие</h2>
                   ...
                   if (!empty($array_complects)):
                        ...
                        <section>
                            <div id="accessories-tabs" class="tabs-wrapp accessories-tabs">
                                ...
                                <?
                                $s = 1;
                                //foreach ($array_complects2 as $sect_id => $comps):
                                    ?>
                                        ...
                                        <div class="items-section">
                                            <div class="items-slider-wrapp slider-wrapp">
                                                <div class="items-slider-body slider-body">
                                                    <div class="items-slider-komplect-more">
                                                    	<div class="items-slider items-slider-komplect-more-wrapp">
                                                    	
                                                    	<div class="tabs-nav tabs-nav-komplect" data-tabs="#accessories-tabs">
                                                    		<ul>
							                                <?
							                                $s = 1;
							                                //print_r($array_complects);
							                                foreach ($array_complects as $sect_id => $comps):
							                                    $sect_arr = CIBlockSection::GetByID($comps['name'])->GetNext();
							                                    ?>
							                                    <li <? /*if ($s == 1): ?>class="active"<? endif; */?>><a data-sect="<?= $comps['name']; ?>" href="#accessories-tab-<?= $comps['name']; ?>"><?= $sect_arr['NAME']; ?></a></li>
							                                    <?
							                                    $s++;
							                                endforeach;
							                                ?>
                                                        ...
                                                        <?
                                                        foreach ($array_complects44 as $comp):
                                                            $item = $comp;
                                                            ?>
                                                        ...
                                                        <? endforeach; ?>
                                                        <?
                                                        foreach ($array_complects55 as $comp):
                                                            $item = $comp;
                                                            ?>
                                                        ...
                                                        <? 
                                                    $allcountcompl = count($array_complects44)+count($array_complects55);
                                                    if($allcountcompl >= 4){ ?>
                                                    <div class="descmore2">Показать все</div>
                                                    <? } 
            ...
            <section class="current" tabname="analogs">
                <div class="tab-trigger"><button><span>Аналоги <i><?if( is_array($array_analogs) && count($array_analogs) > 0 ):?><?=count($array_analogs)?><?else:?>0<?endif?></i></span></button></div>
                <div class="tab-content">
                    <h2 class="product-info-title desktop-only">Аналогичные товары</h2>
                    ...
                    <section id="analogs">
                        <div class="items-section">
                            <div class="items-slider-wrapp slider-wrapp">
                                <div class="items-slider-body slider-body">
                                    <div class="items-slider items-slider-analog-more">
                                        <?
                                    ...
                                    <? 
                                                $allcountanalog = count($array_analogs);
                                                if($allcountanalog > 4){ ?>
                                                <div class="descmore3">Показать все</div>
                                                <? } ?>
                                </div>
                            </div>
                            <!--<div class="items-slider-dots slider-dots"></div>-->
