...
if (!empty($arResult['DISPLAY_PROPERTIES']))
{
	...
	{
		if($arDispProp['ID'] == '475' && !empty($arDispProp['VALUE'])){
			$arFilter = Array('IBLOCK_ID'=>13, 'ACTIVE'=>'Y', 'NAME'=>$arDispProp['VALUE'], '!SECTION_PAGE_URL'=>false);
			$dbList = CIBlockSection::GetList(Array(), $arFilter, true, array('SECTION_PAGE_URL'));
			if($sResult = $dbList->GetNext())
			{
				$arResult['DISPLAY_PROPERTIES'][$propKey]['URL'] = $sResult["SECTION_PAGE_URL"];
			}
		}		
...
if(!empty($arResult['NAME']) && !empty($arResult['ID'])){
	$sMetaName = $arResult['NAME'] . ((!empty($arResult['PROPERTIES']['CML2_ARTICLE']['VALUE'])) ? ' ' . $arResult['PROPERTIES']['CML2_ARTICLE']['VALUE'] : '');
	$sMetaPrice = '';
	$dbPrice = CPrice::GetList(array(), array("PRODUCT_ID" => $arResult['ID'], "CATALOG_GROUP_ID" => 4));
	if ($arPrice = $dbPrice->Fetch()) {
		$sPrice = '';
	    if ($arPrice['CURRENCY'] == 'RUB') {
	        $sPrice = $arPrice['PRICE'];
	    } else {
	        $sPrice = CCurrencyRates::ConvertCurrency($arPrice['PRICE'], $arPrice['CURRENCY'], "RUB");
	        $sPrice = number_format($sPrice, 2, '.', '');
	    }
	    $sMetaPrice = ' по цене ' . $sPrice . ' руб';
	}

	$arResult['META_TAGS']['BROWSER_TITLE'] = $sMetaName . ' купить в Москве - Октант';
	$arResult['META_TAGS']['DESCRIPTION'] = 'Купить ' . $sMetaName . ' в наличии на складе в Москве' . $sMetaPrice . '. Подробное описание, характеристики, документация, низкие цены. Интернет-магазин Октант с быстрой доставкой курьером.';
}
...
$productData = $photoManager->getProductDataWithPhotos($arResult['ID']);
if (!empty($arResult['DETAIL_PICTURE'])) {
    if (isset($arResult['DETAIL_PICTURE']['SRC'])) {
        $productData[0] = $arResult['DETAIL_PICTURE']['SRC'];
    } else {
        $productData[0] = CFile::GetPath($arResult['DETAIL_PICTURE']);
    }
} else {
    if (!empty($arResult['PREVIEW_PICTURE']['SRC'])) {
        $productData[0] = $arResult['PREVIEW_PICTURE']['SRC'];
    } else {
...
$arResult['PHOTO_LIST'] = $productData;
?>