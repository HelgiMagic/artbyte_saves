BX.saleOrderAjax = {
            ...
            if (code == 'slst') {
              ...
              (function (k, control) {
                ...
                if (BX.type.isDomNode(townInputFlag) && townInputFlag.value == '1') {
                  ...
                  if (typeof adapter != 'undefined' && adapter !== null) adapter.setValuePair('other', ctx.options.messages.otherLocation);
                }
              })(k, control);
            }
          }
  ...
  checkAbility: function (propId, ability) {
    ...
    if (typeof this.properties[propId].abilities != 'undefined' && this.properties[propId].abilities[ability]) return true;
  ...
  getAltLocPropByRealLocProp: function (propId) {
    if (typeof this.properties[propId].altLocationPropId != 'undefined') return this.properties[this.properties[propId].altLocationPropId];
...
$(document).ready(function () {
  $(document).on('change', '#licenses_subscribe', function (e) {
    $('[data-property-id-row="22"] input').val(this.checked ? 'Y' : 'N');
    $('[data-property-id-row="22"] input[type="checkbox"]').prop('checked', this.checked);
  });

  $(document).on('change', '#UF_SUBSCRIBE', function (e) {
    $('#UF_SUBSCRIBE').val(this.checked ? '1' : '0');

    $('[data-property-id-row="23"] input').val(this.checked ? 'Y' : 'N');
    $('[data-property-id-row="23"] input[type="checkbox"]').prop('checked', this.checked);
  });

  $(document).on('change', '#purchase_agreement', function (e) {
    $('[data-property-id-row="24"] input').val(this.checked ? 'Y' : 'N');
    $('[data-property-id-row="24"] input[type="checkbox"]').prop('checked', this.checked);
  });

  $(document).on('change', '#purchase_agreement_mobile', function (e) {
    $('[data-property-id-row="24"] input').val(this.checked ? 'Y' : 'N');
    $('[data-property-id-row="24"] input[type="checkbox"]').prop('checked', this.checked);
  });
});
