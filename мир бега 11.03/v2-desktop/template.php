...
$APPLICATION->AddViewContent('cowl_buttons', '<div class="basket-back font_13"><a href="'.TSolution::GetFrontParametrValue('BASKET_PAGE_URL').'" class="basket-back__link muted">'.Loc::getMessage('GO_TO_BASKET').'</a></div>');
?>
	<NOSCRIPT>
...
else
{
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order-main" class="orderform--v2 bx-<?=$arParams['TEMPLATE_THEME']?> grid-list grid-list--gap-column-40" style="opacity: 0">
			<!--	MAIN BLOCK	-->
			<div class="bx-soa">
					...
					<? if ($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'): ?>
						...
						<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
							<div class="bx-soa-section-title-container bx-soa-section-title-container--flex">
								...
								<div class="change-info btn btn-transparent"><?=Loc::getMessage('TITLE_SOA_CHANGE_INFO_SECTION')?></div>
							</div>
							<div class="bx-soa-section-content container-fluid"></div>
						...
						<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?=($hideDelivery ? 'style="display:none"' : '')?>>
							<div class="bx-soa-section-title-container bx-soa-section-title-container--flex">
								<h2 class="bx-soa-section-title"><?=$arParams['MESS_DELIVERY_BLOCK_NAME']?><span class="city_destination"><?=Loc::getMessage('TITLE_SOA_DESTINATION')?></span></h2>
								<div class="change-info btn btn-transparent"><?=Loc::getMessage('TITLE_SOA_CHANGE_INFO_SECTION')?></div>
							</div>
			...
			<div id="bx-soa-total" class="bx-soa-sidebar">
				...
				<div class="bx-soa-total-wrapper">
					<div class="bx-soa-cart-total shadow p-inline p-inline--20 p-block p-block--24 outer-rounded-x"></div>
				</div>
			</div>
		</div>
	</form>
	...
	<script>
		...
		BX.Sale.OrderAjaxComponent.init({
            ...
            isUserAuth: <?=$USER->IsAuthorized() ?: 0 ?>,
		});
	</script>
	<script>
		<?
